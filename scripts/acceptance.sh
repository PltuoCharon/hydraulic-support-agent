...
ck "stats概览栏接入"      "grep -q getStats web/src/views/SelectCenterView.vue"
ck "选型中心页文件存在"  "test -f web/src/views/SelectCenterView.vue"
ck "选型中心路由注册"    "grep -q SelectCenterView web/src/router/index.js"
ck "菜单三入口整合"      "grep -q 选型中心 web/src/App.vue && ! grep -q 'index="/wizard"' web/src/App.vue"
