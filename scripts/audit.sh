...
# hs_agent 项目体检脚本 v2
cd ~/hs_agent || exit 1
DBPW='zyb123'          # ★ 只改这一处：填 hs_user 的真实密码
PASS=0; FAIL=0
ck() {
...
python -c "import fastapi" 2>/dev/null && echo "  [INFO] fastapi 已装" || echo "  [INFO] fastapi 未装"
...
ck "crontab 已配自动备份"    "crontab -l 2>/dev/null | grep -q mysqldump"

echo "===== 3. 密钥安全(问题7) ====="
ck ".env 存在"              "test -f .env"
ck ".env 已被 gitignore"    "grep -qx '.env' .gitignore"
ck "代码无硬编码Key"        "! grep -rnE \"api_key\s*=\s*['\\\"][0-9a-f]{32}\" --include='*.py' app core scripts *.py 2>/dev/null | grep -q ."
ck "git历史无Key泄露"       "! git log -p --all 2>/dev/null | grep -qE '[0-9a-f]{32}\.[0-9A-Za-z]{16}'"

echo "===== 4. 数据库健康(问题4/5/8/10) ====="
Q="mysql -u hs_user -p$DBPW hydraulic_support -N -s -e"
ck "数据库可连接"            "$Q 'SELECT 1' | grep -q 1"
I_NULL=$($Q 'SELECT SUM(intensity IS NULL) FROM support_models')
W_NULL=$($Q 'SELECT SUM(weight IS NULL) FROM support_models')
SUSPECT=$($Q "SELECT COUNT(*) FROM support_models WHERE data_status='suspect'")
echo "  [INFO] 强度缺失: $I_NULL"
echo "  [INFO] 重量缺失: $W_NULL"
echo "  [INFO] suspect数: $SUSPECT"
